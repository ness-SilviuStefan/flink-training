...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        public void open(Configuration config) {
            taxiRideValueState = getState(TaxiRide.class);
            taxiFareValueState = getState(TaxiFare.class);
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            final TaxiFare taxiFare = this.taxiFareValueState.value();
        ...
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
            final TaxiRide taxiRide = this.taxiRideValueState.value();
        ...
        private <T> ValueState<T> getState(Class<T> tClass) {
            return getRuntimeContext().getState(new ValueStateDescriptor<>(tClass.getName(), tClass));
        }
    }
}
